 ...
 *
 * The code in this file emulates that hardware solution using linear probing. ---@11/29/2015
 */
...
static inline int xt_to_vxt_rcu(xid_type_t ty)
{
	if (likely(ty > 0)) {
		...
		entry = &(rcu_dereference(xip_virtual_xid_types)[hash_index]);

		if (atomic_read(&tot_element) == 0) {
			pr_info("Linear Probing Hash Table is Empty!!\n");
			return -1;
		}
		
		while (entry->marker != 0) {
			if (likely(entry->xid_type == ty)) {
				pr_info("The current hash_index is %d\n", hash_index);
			...
			entry = 
				&(
						rcu_dereference(xip_virtual_xid_types)[hash_index]
				 );
		}

		if (!flag)
			pr_info("Given data is not present in hash table\n");
	}
